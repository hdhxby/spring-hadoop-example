package io.github.thinkframework.data.ibatis.builder.annotation;
...
/**
 * 添加EntityLanguageDriver处理逻辑
 * 在createSqlSource上添加mapperType, mapperMethod参数
 * @see EntityLanguageDriver
 * @see org.apache.ibatis.builder.annotation.MapperAnnotationBuilder
 */
public class EntityMapperAnnotationBuilder extends MapperAnnotationBuilder {
    ...
    public EntityMapperAnnotationBuilder(Configuration configuration, Class<?> type) {
    ...
    private SqlSource buildSqlSource(Annotation annotation, Class<?> parameterType, LanguageDriver languageDriver,
        ...
        if (annotation instanceof Select) {
            return buildSqlSourceFromStrings(((Select) annotation).value(), parameterType, type, method, languageDriver);
        } else if (annotation instanceof Update) {
            return buildSqlSourceFromStrings(((Update) annotation).value(), parameterType, type, method,  languageDriver);
        } else if (annotation instanceof Insert) {
            return buildSqlSourceFromStrings(((Insert) annotation).value(), parameterType, type, method,  languageDriver);
        } else if (annotation instanceof Delete) {
            return buildSqlSourceFromStrings(((Delete) annotation).value(), parameterType, type, method,  languageDriver);
        } else if (annotation instanceof SelectKey) {
            return buildSqlSourceFromStrings(((SelectKey) annotation).statement(), parameterType, type, method,  languageDriver);
        }
        return new ProviderSqlSource(assistant.getConfiguration(), annotation, type, method);
    }

    private SqlSource buildSqlSourceFromStrings(String[] strings, Class<?> parameterTypeClass, Class<?> mapperType, Method mapperMethod,
                                                LanguageDriver languageDriver) {
        if(languageDriver instanceof EntityLanguageDriver) { // 自定义逻辑, 在原生实现的技术上添加mapperType, mapperMetho参数
            return ((EntityLanguageDriver) languageDriver).createSqlSource(configuration, String.join(" ", strings).trim(), parameterTypeClass, mapperType, mapperMethod);
        }
